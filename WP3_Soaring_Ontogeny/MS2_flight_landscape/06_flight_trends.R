...
################################################## redo above for other variables ##########################################################################
...
flight_summary <- lapply(flight, function (x){
  ...
  #calculate the ratio of each mode of flight to total flight time (as number of rows within a burst)
  ratios <- x %>% 
    mutate(behavior = factor(behavior, levels = c("Flapping", "NotFlapping", "Unclassified"))) %>% 
    group_by(local_identifier, weeks_since_emig, days_since_emig, burstID, behavior) %>% 
    summarize(n_rows = length(behavior)) %>% 
    group_by(local_identifier, weeks_since_emig, days_since_emig, burstID) %>% 
    complete(behavior, fill = list(n_rows = 0)) %>% #add zero for instances where a factor level was not observed in a burst
    mutate(total_flight_in_burst = sum(n_rows)) %>% 
    mutate(flight_type_to_all = n_rows/total_flight_in_burst,
           no_flap_to_flap = n_rows[behavior == "NotFlapping"]/ n_rows[behavior == "Flapping"]) #calculate the ratio of thermal soaring to linear soaring. 19 bursts have neither ciclring nor linear soaring 
