...
graph_files <- list.files("/home/enourani/ownCloud/Work/cluster_computing/GE_inla_static/results/weekly_Feb23/weekly_output", pattern = "graph*", full.names = T)


graph_df <- lapply(graph_files, function(x){
  
  graph <- readRDS(x) %>% 
    mutate(week = as.numeric(str_sub(x, 117,-5))) #extract 

  }) %>% 
  reduce(rbind)

#for lapply run on pc
graph_df <- graph_ls %>% 
  reduce(rbind)

X11(width = 11, height = 4)
coeffs <- ggplot(graph_df, aes(x = week, y = Estimate, group = Factor, color = Factor)) +
...
ggplot(graph_df %>% filter(Factor == "dem_100_z"), aes(x = week, y = Estimate)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_line() + 
  geom_point() +
  #ylim(c(-2.5, 2.5)) +
...
ggplot(graph_df %>% filter(Factor == "TRI_100_z"), aes(x = week, y = Estimate)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_line() + 
  geom_point() +
  #ylim(c(-2.5, 2.5)) +
